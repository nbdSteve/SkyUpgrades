...
import gg.steve.skyupgrades.nbt.NBTContainer;
...
public class ItemBuilderUtil {
    ...
    public void addData(int track, int tier, boolean unbreakable) {
        this.nbtItem = new NBTItem(item);
        if (unbreakable) {
            nbtItem.setBoolean("Unbreakable", true);
        }
        nbtItem.setBoolean("sky-upgrades.tool", true);
        nbtItem.setInteger("sky-upgrades.track", track);
        nbtItem.setInteger("sky-upgrades.tier", tier);
    }
